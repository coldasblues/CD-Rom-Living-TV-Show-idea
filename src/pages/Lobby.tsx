...
const Lobby: React.FC = () => {
  ...
  // Preview State
  const [previewTape, setPreviewTape] = useState<StoredTape | null>(null);
  ...
  const onDrop = async (e: DragEvent) => {
    ...
    const processedTapes: StoredTape[] = [];
    ...
    if (processedTapes.length === 1) {
        setPreviewTape(processedTapes[0]);
    } else if (processedTapes.length > 1) {
        for (const tape of processedTapes) {
            await saveTapeToLibrary(tape);
        }
        setLibrary(prev => [...processedTapes, ...prev]);
    }
  };

  const handleConfirmPreview = async () => {
      if (!previewTape) return;
      await saveTapeToLibrary(previewTape);
      setLibrary(prev => [previewTape, ...prev]);
      setPreviewTape(null);
  };
    ...
    <div className="min-h-screen w-full bg-[#050505]" onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
      <CRTContainer>
        <div className="flex flex-col h-full w-full p-6 relative">
                ...
                <div className="p-8 max-w-2xl mx-auto w-full overflow-y-auto">
                    <div className="mb-8 border border-green-900 p-6 bg-black/50">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <p className="text-xs text-gray-600">Enter Fal key to use Minimax/Luma models or Slideshow Mode.</p>
                    ...
                    <div className="mb-8 border border-green-900 p-6 bg-black/50">
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-green-800 text-sm">RENDER MODE</label>
                                <select 
                                    value={settings.globalRenderMode || 'video'}
                                    onChange={(e) => savePreferences({...settings, globalRenderMode: e.target.value as 'video' | 'slideshow'})}
                                ...
                                >
                                    <option value="video">Video (Cinematic / Slow)</option>
                                    <option value="slideshow">Slideshow (Fast / Static Images)</option>
                                </select>
                                <p className="text-xs text-gray-600">
                                    Slideshow mode generates static images using Fal.ai (Flux) instead of video. Much faster and cheaper. Requires Fal Key.
                                </p>
                            </div>
          ...
          {/* TAPE PREVIEW MODAL */}
          {previewTape && (
            <div className="absolute inset-0 z-50 bg-black/95 flex items-center justify-center p-8 font-mono">
               <div className="w-full max-w-3xl border-2 border-green-500 p-1 bg-[#050505] shadow-[0_0_30px_rgba(0,255,0,0.2)] relative">
                  <div className="absolute top-0 left-0 bg-green-500 text-black text-xs font-bold px-2 py-1">
                      NEW DATA DETECTED
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 pt-8">
                      {/* Image */}
                      <div className="aspect-[3/4] border border-green-900 bg-black relative group">
                          <img 
                            src={`data:image/png;base64,${previewTape.imgBase64}`} 
                      ...
                      <div className="md:col-span-2 flex flex-col justify-between">
                          <div>
                              <h2 className="text-3xl text-green-500 font-bold mb-1 tracking-wider text-glow">{previewTape.characterName}</h2>
                              <p className="text-green-800 text-sm uppercase mb-4">
                                  Author: {previewTape.data.meta.author || 'Unknown'} • Ver: {previewTape.data.meta.version || '1.0'}
                              </p>
                              
                              <div className="border-t border-green-900/50 pt-4 mb-4">
                                  <h3 className="text-xs text-green-700 uppercase mb-2">Log Snippet</h3>
                                  <p className="text-green-600 text-sm leading-relaxed opacity-80 line-clamp-6">
                                      {previewTape.data.engineState.currentBeat?.narrative || "No description data available."}
                                  </p>
                              </div>

                              {previewTape.data.meta.visualStyle && (
                                  <p className="text-xs text-green-800 uppercase">
                                      Visual Style: <span className="text-green-500">{previewTape.data.meta.visualStyle}</span>
                                  </p>
                              )}
                          </div>
                          ...
                          <div className="flex gap-4 mt-6">
                              <button 
                                ...
                                className="flex-1 bg-green-700 hover:bg-green-600 text-black font-bold py-3 uppercase tracking-widest transition-all"
                              >
                                  [ Import Tape ]
                              </button>
                              <button 
                                onClick={handleCancelPreview}
                                className="px-6 border border-green-900 text-green-700 hover:text-red-500 hover:border-red-900 py-3 uppercase tracking-widest transition-all"
                              >
                                  Discard
                              </button>
                          </div>
