...
const TapeDeck = forwardRef<TapeDeckHandle, TapeDeckProps>(({ videoSrc, staticImageSrc, onEnded, isProcessing, loop = false }, ref) => {
  ...
  // --- STATIC NOISE GENERATOR (Web Audio API) ---
  useEffect(() => {
    ...
    if (isProcessing) {
        try {
            ...
            if (AudioContext) {
                ...
                gainNode.gain.value = 0.05; // Low volume static texture
            ...
            console.warn("Audio Context blocked (user interaction needed):", e);
        }
    }
    ...
    <div className="relative w-full aspect-video bg-black flex items-center justify-center overflow-hidden border-b-2 border-gray-800 group">
      {videoSrc ? (
        <video
          ...
          onEnded={onEnded}
          loop={loop}
          muted={isProcessing} // Mute video track during processing (we use generated noise), unmute for content
          autoPlay
        >
